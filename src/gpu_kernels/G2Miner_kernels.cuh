...
template <typename VID, typename VLABEL>
__global__ void motif_G2Miner(VID ne, dev::Graph<VID, VLABEL> g, VID *vlists,
                              VID max_deg, AccType *counters) {
  ...
  for (VID eid = warp_id; eid < ne; eid += num_warps) {
    ...
    // finding 3-star
    auto cnt = difference_set(v0_ptr, v0_size[warp_lane], v1_ptr,
                              v1_size[warp_lane], v1[warp_lane], vlist);
    if (thread_lane == 0)
      list_size[warp_lane][0] = cnt;
    __syncwarp();
    for (VID i = 0; i < list_size[warp_lane][0]; i++) {
      ...
      counts[0] += difference_num(vlist, list_size[warp_lane][0],
    ...
    cnt = intersect(v0_ptr, v0_size[warp_lane], v1_ptr, v1_size[warp_lane],
                    vlist); // y0y1
    if (thread_lane == 0)
      list_size[warp_lane][0] = cnt;
    __syncwarp();
    for (VID j = 0; j < list_size[warp_lane][0]; j++) {
      v2 = vlist[j];
      v2_size = g.getOutDegree(v2);
      counts[4] += difference_num(vlist, list_size[warp_lane][0],
                                  g.getNeighbor(v2), v2_size, v2); // diamond

      cnt = difference_set(g.getNeighbor(v2), v2_size, v0_ptr,
                           v0_size[warp_lane], &vlist[max_deg]); // n0y2
      if (thread_lane == 0)
        list_size[warp_lane][1] = cnt;
      __syncwarp();
      counts[2] +=
          difference_num(&vlist[max_deg], list_size[warp_lane][1], v1_ptr,
                         v1_size[warp_lane]); // n0n1y2: tailed_triangle
    }

    // finding 4-clique
    cnt = intersect(v0_ptr, v0_size[warp_lane], v1_ptr, v1_size[warp_lane],
                    v1[warp_lane], vlist); // y0f0y1f1
    if (thread_lane == 0)
      list_size[warp_lane][0] = cnt;
    __syncwarp();
    for (VID i = 0; i < list_size[warp_lane][0]; i++) {
      v2 = vlist[i];
      v2_size = g.getOutDegree(v2);
      counts[5] += intersect_num(vlist, list_size[warp_lane][0],
                                 g.getNeighbor(v2), v2_size, v2); // 4-cycle
    }
    ...
    for (VID j = 0; j < list_size[warp_lane][0]; j++) {
      v2 = vlist[j];
      v2_size = g.getOutDegree(v2);
      counts[1] += difference_num(&vlist[max_deg], list_size[warp_lane][1],
                                  g.getNeighbor(v2), v2_size); // 4-path
    }
