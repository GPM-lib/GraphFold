...
void G2MinerSolverMultiGPU(project_GraphFold::Graph<VID, VLABEL> &hg,
    ...
    threads.push_back(std::thread([&, i]() {
...

      {
        if (p.get_name() == "P1") {
          ...
          H_ERR(cudaOccupancyMaxPotentialBlockSize(&grid_size, &block_size,
                                                   clique4_G2Miner<VID, VLABEL>,
                                                   0, (int)MAX_BLOCK_SIZE));
        } else if (p.get_name() == "CF5") {
          H_ERR(cudaOccupancyMaxPotentialBlockSize(&grid_size, &block_size,
                                                   clique5_G2Miner<VID, VLABEL>,
                                                   0, (int)MAX_BLOCK_SIZE));
        } else if (p.get_name() == "CF6") {
          H_ERR(cudaOccupancyMaxPotentialBlockSize(&grid_size, &block_size,
                                                   clique6_G2Miner<VID, VLABEL>,
                                                   0, (int)MAX_BLOCK_SIZE));

        } else if (p.get_name() == "CF7") {
          H_ERR(cudaOccupancyMaxPotentialBlockSize(&grid_size, &block_size,
                                                   clique7_G2Miner<VID, VLABEL>,
                                                   0, (int)MAX_BLOCK_SIZE));
      ...
      if (p.get_name() == "P1") {
        P1_G2Miner<VID, VLABEL><<<grid_size, block_size>>>(
            tasks[i], d_g, d_frontier_list, max_degree, d_total[i]);
      } else if (p.get_name() == "P2") {
        ...
        tc_G2Miner<VID, VLABEL>
            <<<grid_size, block_size>>>(tasks[i], d_g, d_total[i]);
      } else if (p.get_name() == "MC4") {
  ...
  if (p.get_name() != "MC4") {
    for (int i = 0; i < n_dev; ++i) {
      std::cout << p.get_name() << " from GPU[" << i << "] is "
                << h_total[i * count_length] << std::endl;
      result += h_total[i * count_length];
    }
