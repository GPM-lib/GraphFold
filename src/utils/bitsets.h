...
class Bitsets {
  public:
    ...
    void alloc(int n, int nbits) {
      ...
      for (int i = 0; i < n; i++) {
        H_ERR(cudaMalloc(&h_bit_vectors[i], vec_size() * sizeof(BSType)));
        reset(i);
      }
      H_ERR(cudaMalloc(&d_bit_vectors, n * sizeof(BSType*)));
      H_ERR(cudaMemcpy(d_bit_vectors, h_bit_vectors, n * sizeof(BSType*), cudaMemcpyHostToDevice));
    }
    void clear() {
      for (int i = 0; i < num_sets; i++) reset(i);
      H_ERR(cudaMemcpy(d_bit_vectors, h_bit_vectors, num_sets * sizeof(BSType*), cudaMemcpyHostToDevice));
    }
    void clean() {
    ...
    void reset(int i) {
      H_ERR(cudaMemset(h_bit_vectors[i], 0, vec_size() * sizeof(BSType)));
    }
...
std::cout << m << " bitsets of " << n << " sets, " << num_chunks << " chunks, " << nbits 
      ...
      H_ERR(cudaMalloc(&begin, mem_size));
      H_ERR(cudaMemset(begin, 0, mem_size));
    ...
    __device__ int intersect_num(size_t offset, int nc, int sid1, int sid2) {
     ...
     __device__ int difference_numA(size_t offset, int nc, int sid1, int sid2) {
      T* v1 = begin + offset + sid1 * num_chunks;
      T* v2 = begin + offset + sid2 * num_chunks;
      int num = 0;
      int thread_lane = threadIdx.x & (WARP_SIZE-1);
